...
% Basin Boundary at H = 0.2
%H4 = hosingfunc(t,0, 5000,1,1,0.2);
%t4 = 5000:-1:0;
%Dt4 = -1;
%Sinput4 = [(-0.042/100)+p.S0, (0.1272/100)+p.S0];%just above saddle point
%SxEM4 = ThreeboxEMvectors(L,N,Dt4,t4,Sinput4,p,H4);
%SxEMpsu4 = SxEM4*1000;
%Sinput5 = [(-0.042/100)+p.S0,(0.1268/100)+p.S0]; %just below saddle point
%SxEM5 = ThreeboxEMvectors(L,N,Dt4,t4,Sinput5,p,H4);
%SxEMpsu5 = SxEM5*1000;
...
%t6 = 5000:-1:0;
%Dt6 = -1;
%Sinput6 = [(-0.06/100)+p.S0, (0.15/100)+p.S0];%just above saddle point
%SxEM6 = ThreeboxEMvectors(L,N,Dt6,t6,Sinput6,p,H6);
%SxEMpsu6 = SxEM6*1000;
%Sinput7 = [(-0.062/100)+p.S0,(0.15/100)+p.S0]; %just below saddle point
...
%y1 = yline(0.2317,'--','LineWidth',1.2);
% yl.LabelHorizontalAlignment = 'left';
xlabel('Time (years)');
ylabel({'Freshwater';'Forcing (Sv)'});
%text(250,0.26,'Upper Fold','Color',grayColor);
text(0.005, 0.9,'(a)','Units','normalized');
set(gca,'LineWidth',1.2,'FontSize',12);
pbaspect([15 2 1])
%ylim([0 1]);
box off;
% savefigure_RRC('Bifurcation_tipping_HadGEM3MM_Hosing_3',3,printfigs)
hold off;
...
% axis square;
% savefigure_RRC('Bifurcation_tipping_HadGEM3MM_3',4,printfigs)
hold off;
...
% xlabel('\boldmath $Time (years)$','Interpreter','latex','fontsize',12);
% ylabel('\boldmath $S_{N} (psu)$','Interpreter','latex','fontsize',12);
% xlabel('Time (years)');
xlabel('Time (years)');
ylabel({'Box';'salinities (psu)'});
% axis([500 2500 32.5 36]);
set(gca,'LineWidth',1.2,'FontSize',12);
text(0.005, 0.9,'(b)','Units','normalized');
pbaspect([15 2 1])
printfigs = false;
savefigure_RRC('FAMOUSA_slow_hosing_combined',1,printfigs)
